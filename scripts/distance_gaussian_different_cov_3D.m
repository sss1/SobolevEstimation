...
true_distance_gaussian_cov = integral3(square_difference,-inf,inf,-inf,inf,-inf,inf);
estimated_distances_gaussian_cov = zeros(n_repeated,total_order_size);
s = 0;
s2= 4;
...
for j = 1:n_repeated
    ...
    for i = 1:total_order_size
        ...
        Ys = 2*randn(n_samples(i),D);
        estimated_distances_gaussian_cov(j,i) = real(sobolev_distance_est(Xs, Ys, s, Zs(n_samples(i))))/(2*pi)^D;
    end
end

end_time = toc(start_time);

save('./distance_gaussian_different_cov_3D.mat');

%plot
errorbar_gaussian_mean = quantile(estimated_distances_gaussian_cov,3);
true_gaussian_mean_multiples = repmat(true_distance_gaussian_cov,[1,total_order_size]);
figure;
hold on;
...
set(gca,'FontSize',20);
saveas(gcf,'./plots/distance_gaussian_different_cov_3D','fig');
export_fig(gcf,'./plots/distance_gaussian_different_cov_3D.pdf');
